...
const SERVICE_ACCOUNT_EMAIL = "techflow-sheets-service@athena-assignment.iam.gserviceaccount.com";
...
export function GoogleSheetsDialog({ isOpen, onClose }: GoogleSheetsDialogProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("techflow-spreadsheet-id");
    if (saved) {
  ...
  useEffect(() => {
    if (spreadsheetId) {
      localStorage.setItem("techflow-spreadsheet-id", spreadsheetId);
    }
  }, [spreadsheetId]);
